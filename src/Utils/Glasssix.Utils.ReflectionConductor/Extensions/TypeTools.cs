...
namespace Extensions
{
    ...
    public static class TypeTools
    {
        ...
        public static Type[] GetImplementsType<T>(BaseTypeMode baseTypeMode)
        {
            ...
            switch (baseTypeMode)
            {
                case BaseTypeMode.Interface:
                    typeQuery.Where(t => t.IsInterface && !t.IsAbstract && !t.IsClass).ToArray();
                    break;
                case BaseTypeMode.Abstract:
                    typeQuery.Where(t => !t.IsInterface && t.IsAbstract && !t.IsClass).ToArray();
                    break;
                case BaseTypeMode.Class:
                    typeQuery.Where(t => !t.IsInterface && !t.IsAbstract && t.IsClass).ToArray();
                    break;
                default:
                    throw new ArgumentOutOfRangeException(nameof(baseTypeMode), baseTypeMode, null);
        ...
        {
            return StringTools.Join(list, quotes, separator);
        }
