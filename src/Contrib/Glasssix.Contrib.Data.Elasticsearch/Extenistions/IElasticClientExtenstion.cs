...
using Glasssix.Contrib.Data.Elasticsearch.Converters;
...
namespace Glasssix.Contrib.Data.Elasticsearch.Extenistions
{
    internal static class IElasticClientExtenstion
    {
            ...
            Action<ISearchResponse<TResult>, TQuery> resultFunc) where TResult : class where TQuery : class
        {
            ...
            {
                throw new Exception($"SearchAsync execute error {ex.Message}");
            }
        }
        ...
        {
            if (!response.IsValid)
                throw new Exception($"elastic query error: status: {response.ServerError?.Status}, message: {response.OriginalException?.Message ?? response.ServerError?.ToString()}, DebugInformation: {response.DebugInformation}");
        }
        ...
        {
            var result = query.Conditions?.ToList() ?? new List<FieldConditionDto>();
        ...
        private static SearchDescriptor<TResult> AddPageSize<TResult, TQuery>(this SearchDescriptor<TResult> container, TQuery query, bool hasPage) where TResult : class where TQuery : BaseRequestDto
        {
            ...
            if (ElasticConstant.MaxRecordCount - start - query.PageSize <= 0)
                throw new Exception($"elastic query data max count must be less {ElasticConstant.MaxRecordCount}, please input more condition to limit");
                ...
                case AggregateTypes.DateHistogram:
                    {
                        ...
                        {
                            throw new Exception($"Field of type [{field}] is not supported for aggregation [date_histogram]");
                        }
