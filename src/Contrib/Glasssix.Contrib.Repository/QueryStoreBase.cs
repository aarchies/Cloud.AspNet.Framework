...
namespace Glasssix.Contrib.Repository
{
    ...
    public abstract class QueryStoreBase<TEntity> : QueryStoreBase<TEntity, string>, IQueryStore<TEntity> where TEntity : class, IKey<string>
    {
        /// <summary>
        /// 初始化查询存储器
        /// </summary>
    ...
    public abstract class QueryStoreBase<TEntity, TKey> : IQueryStore<TEntity, TKey> where TEntity : class, IKey<TKey>
    {
        ...
        /// </summary>
        /// <param name="unitOfWork">工作单元</param>
        public QueryStoreBase(IUnitOfWork unitOfWork)
        {
            UnitOfWork = (UnitOfWorkBase)unitOfWork;
        }

        public IQueryable<TEntity> Entities => Set.AsQueryable().AsNoTracking();

        /// <summary>
        /// 数据库连接
        /// </summary>
        protected IDbConnection Connection => UnitOfWork.GetDbContext().Database.GetDbConnection();

        /// <summary>
        /// 实体集
        /// </summary>
        protected DbSet<TEntity> Set => UnitOfWork.GetDbContext().Set<TEntity>();
        ...
        /// <returns></returns>
        public virtual List<TEntity> FindByIds(long[] ids)
        {
            return FindByIds(ids.To<List<TKey>>());
        }
        ...
        /// </summary>
        /// <param name="ids">逗号分隔的标识列表，范例："1,2"</param>
        public virtual async Task<List<TEntity>> FindByIdsAsync(long[] ids)
        {
            var idList = ids.To<List<TKey>>();
            return await FindByIdsAsync(idList);
        }
