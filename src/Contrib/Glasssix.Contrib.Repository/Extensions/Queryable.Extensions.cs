...
namespace Glasssix.Contrib.Repository.Extensions
{
    /// <summary>
    /// 查询扩展
    /// </summary>
    public static partial class Extensions
    {
        ...
        public static IQueryable<TEntity> Page<TEntity>(this IQueryable<TEntity> source, IPager pager)
        {
            ...
            QueryHelper.InitOrder(source, pager);
            if (pager.Total <= 0)
                pager.Total = source.Count();
            var orderedQueryable = QueryHelper.GetOrderedQueryable(source, pager);
        ...
        public static async Task<IQueryable<TEntity>> PageAsync<TEntity>(this IQueryable<TEntity> source, IPager pager)
        {
            ...
            QueryHelper.InitOrder(source, pager);
            if (pager.Total <= 0)
                pager.Total = await source.CountAsync();
            var orderedQueryable = QueryHelper.GetOrderedQueryable(source, pager);
            if (orderedQueryable == null)
                throw new ArgumentException("必须设置排序字段");
            return orderedQueryable.Skip(pager.GetSkipCount()).Take(pager.PageSize);
