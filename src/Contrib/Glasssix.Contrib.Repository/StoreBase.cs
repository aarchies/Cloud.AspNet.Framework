...
namespace Glasssix.Contrib.Repository
{
    ...
    public abstract class StoreBase<TEntity> : StoreBase<TEntity, string>, IStore<TEntity>
        where TEntity : class, IKey<string>
    {
        ...
        public StoreBase(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
        }
    }
    ...
    public abstract class StoreBase<TEntity, TKey> : QueryStoreBase<TEntity, TKey>, IStore<TEntity, TKey> where TEntity : class, IKey<TKey>
    {
        ...
        public virtual void Update(TEntity entity)
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));
            UnitOfWork.GetDbContext().Entry(entity).State = EntityState.Detached;
            var old = Find(entity.Id);
            var oldEntry = UnitOfWork.GetDbContext().Entry(old);
            oldEntry.State = EntityState.Detached;
            oldEntry = UnitOfWork.GetDbContext().Attach(old);
