...
namespace Glasssix.Contrib.Domain.Shared.Pager
{
    ...
    public class PagerList<T> : IPagerBase
    {
        ...
        public PagerList(int page, int pageSize, int totalCount, string order, IEnumerable<T>? data = null)
        {
            ...
            //处理行序号
            //if (Data.Any() && Data[0] is LineNoBase)
            //{
            //    var line = (Page - 1) * PageSize;
            //    for (var i = 0; i < Data.Count; i++)
            //    {
            //        (Data[i] as LineNoBase).LineNo = line + i + 1;
            //    }
            //}
        }
        ...
        {
            return new PagerList<TResult>(Page, PageSize, Total, Order, data);
        }
    }
}