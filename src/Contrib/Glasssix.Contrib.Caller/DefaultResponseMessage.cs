    ...
    public abstract class DefaultResponseMessage : IResponseMessage
    {
        ...
        public static async Task ProcessResponseExceptionAsync(HttpResponseMessage response, CancellationToken cancellationToken = default)
        {
            if (response.Content.Headers.ContentLength is > 0)
                throw new DataException(await response.Content.ReadAsStringAsync());

            throw new DataException($"ReasonPhrase: {response.ReasonPhrase ?? string.Empty}, StatusCode: {response.StatusCode}");
        }
        ...
        public virtual async Task ProcessResponseAsync(HttpResponseMessage response, CancellationToken cancellationToken = default)
        {
            ...
            switch (response.StatusCode)
            {
                //case (HttpStatusCode)GlasssixHttpStatusCode.UserFriendlyException:
                //    throw new UserFriendlyException(await response.Content.ReadAsStringAsync());
                //case (HttpStatusCode)GlasssixHttpStatusCode.ValidatorException:
                //    throw new GlasssixValidatorException(await response.Content.ReadAsStringAsync());
                default:
