...
namespace Glasssix.Contrib.Caller
{
    public abstract class AbstractCaller : ICaller
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var GlasssixHttpContext = new GlasssixHttpContext(ResponseMessage, () => CreateRequest(HttpMethod.Get, methodName));
            CallerHandlerDelegate callerHandlerDelegate = async () =>
            {
                GlasssixHttpContext.ResponseMessage = await SendAsync(GlasssixHttpContext.RequestMessage, cancellationToken);

                if (autoThrowException) await GlasssixHttpContext.ProcessResponseAsync(cancellationToken);

                content = await GlasssixHttpContext.ResponseMessage.Content.ReadAsByteArrayAsync();
            };
            await Middlewares.Aggregate(callerHandlerDelegate,
                (next, func) => () => func.Invoke(ServiceProvider).HandleAsync(GlasssixHttpContext, next, cancellationToken))();
            return content;
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            CallerHandlerDelegate callerHandlerDelegate = async () =>
            {
                ...
                content = await GlasssixHttpContext.ResponseMessage.Content.ReadAsStreamAsync();
            };
            await Middlewares.Aggregate(callerHandlerDelegate,
                (next, func) => () => func.Invoke(ServiceProvider).HandleAsync(GlasssixHttpContext, next, cancellationToken))();
            return content;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            CallerHandlerDelegate callerHandlerDelegate = async () =>
            {
                ...
                content = await GlasssixHttpContext.ResponseMessage.Content.ReadAsStringAsync();
            };
        ...
        protected virtual async Task<HttpResponseMessage> SendAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var GlasssixHttpContext = new GlasssixHttpContext(ResponseMessage, requestMessageFunc);
            CallerHandlerDelegate callerHandlerDelegate = async () =>
            {
                GlasssixHttpContext.ResponseMessage = await SendAsync(GlasssixHttpContext.RequestMessage, cancellationToken);
            ...
            await Middlewares.Aggregate(callerHandlerDelegate,
                (next, func) => () => func.Invoke(ServiceProvider).HandleAsync(GlasssixHttpContext, next, cancellationToken))();
            return GlasssixHttpContext.ResponseMessage!;
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                GlasssixHttpContext.ResponseMessage = await SendAsync(GlasssixHttpContext.RequestMessage, cancellationToken);
                response = await GlasssixHttpContext.ProcessResponseAsync<TResponse>(cancellationToken);
            };
            await Middlewares.Aggregate(callerHandlerDelegate,
                (next, func) => () => func.Invoke(ServiceProvider).HandleAsync(GlasssixHttpContext, next, cancellationToken))();
            return response;
