using Glasssix.BuildingBlocks.Caching.Distributed.StackExchangeRedis.Control.Internal.Enumerations;
...
namespace Glasssix.BuildingBlocks.Caching.Distributed.StackExchangeRedis.Control.Internal.Helpers
{
    internal static class RedisHelper
    {
        public static RedisValue ConvertFromValue<T>(this T value, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            dynamic redisValue;
            switch (type.GetCompressMode(out Type actualType))
            {
        ...
        {
            var type = typeof(T);
            var compressMode = type.GetCompressMode(out Type actualType);
